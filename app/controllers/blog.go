...
from article right join art_tag on article.id = art_tag.art_id and  article.owner = ? and art_tag.art_tag = ? where article.title is not NULL;`
	findByIdSQL   = `select  owner, title, location from article where id = ? ;`
...
func (c Blog) Save(title, id, md string) revel.Result {
	...
	err = os.MkdirAll(filepath.Join(articlePathPrefix, lastLoginInfo.Name), os.ModePerm)
	if err != nil {
		return c.RenderError(err)
	}
	path := filepath.Join(articlePathPrefix, lastLoginInfo.Name, id)
	...
	result, err := updateStmt.Exec(title, path, id)
	if err != nil {
		return c.RenderError(err)
	}
